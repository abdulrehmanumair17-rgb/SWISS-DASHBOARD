...
export const DataEntry: React.FC<DataEntryProps> = ({ data, onDataUpdate }) => {
  const [isUploading, setIsUploading] = useState(false);
  const [uploadError, setUploadError] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<'master' | 'daily' | 'config'>('daily');
  const [holidays, setHolidays] = useState<number[]>([]); 
  const [isHolidaysLocked, setIsHolidaysLocked] = useState(false);
  const [isAdminMode, setIsAdminMode] = useState(false);
  
  const [viewDate, setViewDate] = useState(new Date());

  const isHolidaysEditable = isAdminMode || !isHolidaysLocked;
  ...
  const workingDays = useMemo(() => {
    ...
    while (date.getMonth() === selectedMonth) {
      ...
      const isMarkedHoliday = holidays.includes(dayNum);
      if (!isSunday && !isMarkedHoliday) count++;
  ...
  const handleLockHolidays = () => {
    if (confirm(`Confirm ${workingDays} working days for ${monthName}? Once finalized, dates can only be changed by Admin.`)) {
      setIsHolidaysLocked(true);
    }
  };
  ...
  const handleUpload = (e: React.ChangeEvent<HTMLInputElement>, type: 'master' | 'daily') => {
    ...
    reader.onload = (evt) => {
      try {
          ...
          if (foundTeam) {
            ...
            if (!isNaN(val)) {
              entries.push({
                ...
                team: foundTeam,
                metric: officialName,
                plan: type === 'master' ? val : 0,
                actual: type === 'daily' ? val : 0,
                variance: 0,
                unit: 'Units',
                status: 'on-track',
                reportDate: type === 'daily' ? extractedDate : undefined
              });
            }
          }
        });
        ...
        if (entries.length > 0) {
          const otherData = type === 'daily' 
            ? data.filter(d => d.reportDate !== extractedDate)
            : data.filter(d => d.reportDate !== undefined || d.department !== 'Sales');
          
          onDataUpdate([...otherData, ...entries]);
          setUploadError(null);
          alert(`SUCCESS: 'Sales' ${type === 'master' ? 'Master Plan' : 'Daily Achievement'} synced.`);
        } else {
          setUploadError("IMPORT FAILED: No product matches found in Column 1 of 'Sales' sheet.");
        }
      } catch (err) {
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                {!isHolidaysEditable ? '🔒 CALENDAR LOCKED' : 'Select holidays for this reporting cycle'}
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-7 gap-2">
          ...
          {daysArray.map((day, idx) => {
            ...
            return (
              <button
                key={idx}
                disabled={isSunday || !isHolidaysEditable}
                onClick={() => toggleHoliday(day)}
                className={`h-14 rounded-xl text-xs font-black transition-all flex items-center justify-center relative ${
                  isSunday ? 'bg-slate-100 text-slate-300 cursor-not-allowed opacity-50' :
                  isHoliday ? 'bg-red-600 text-white shadow-lg shadow-red-100 scale-95' :
                  !isHolidaysEditable ? 'bg-slate-50 text-slate-400 border border-slate-100 cursor-not-allowed' :
        ...
        <div className="pt-8 flex flex-col items-center gap-4">
          {isHolidaysEditable ? (
            ...
            <div className="flex flex-col items-center gap-2">
              <div className="px-6 py-3 bg-green-50 border border-green-200 text-green-700 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clipRule="evenodd" /></svg>
                Operational Calendar Finalized
              </div>
              <p className="text-[8px] text-slate-400 font-bold uppercase tracking-widest">Only administrators can modify this schedule now</p>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="max-w-[1200px] mx-auto space-y-12 pb-20">
      <div className="flex flex-col items-center text-center space-y-4">
        ...
        <div className="flex items-center gap-6">
          <p className="text-slate-500 font-medium italic">Operational data synchronization gateway.</p>
          <button 
            onClick={handleAdminToggle}
            className={`px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest transition-all border ${isAdminMode ? 'bg-amber-100 border-amber-300 text-amber-700' : 'bg-slate-100 border-slate-200 text-slate-400 hover:text-slate-900'}`}
          >
      ...
      <div className="bg-white rounded-[4rem] border border-slate-200 shadow-2xl overflow-hidden">
        <div className="flex border-b border-slate-100 bg-slate-50/50">
          ...
          <button onClick={() => { setActiveTab('config'); setUploadError(null); }} className={`flex-1 py-8 text-[10px] font-black uppercase tracking-[0.3em] transition-all ${activeTab === 'config' ? (isHolidaysLocked ? 'bg-green-800 text-white' : 'bg-green-700 text-white') : 'text-slate-400 hover:bg-slate-100'}`}>Working Days</button>
        </div>
            ...
            <div className="flex flex-col items-center space-y-10">
              ...
              <div className="text-center space-y-2">
                <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight italic">
                  {activeTab === 'daily' ? 'Daily Sync' : 'Master Audit Sync'}
                </h3>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Excel Literal Cell Extraction - Sales Only</p>
              ...
              <div className="w-full max-w-lg min-h-[60px] flex flex-col items-center justify-center text-center space-y-3">
                {uploadError ? (
                  ...
                  <div className="text-[9px] text-slate-400 font-bold uppercase tracking-tight">
                    {activeTab === 'daily' 
                      ? "Requires header: 'Actual' or 'Achievement' + Valid Date" 
                      : "Requires header: 'Target' or 'Plan'"}
                  </div>
                )}
              </div>
            </div>
